...
export class DxfViewer {
  ...
  //   // Find intersections
  //   const intersects = raycaster.intersectObjects([boundingBoxMesh], true);

  //   if (intersects.length > 0) {
  //     // Intersection found
  //     const intersectPoint = intersects[0].point;
  //     helperBox.position.copy(intersectPoint);
  ...
  _TransformVertex(v, blockCtx = null) {
    ...
    const finalVertex = { x: v.x - this.origin.x, y: v.y - this.origin.y };
    /* It looks like the code is attempting to write a comment in JavaScript using the `/* */

    return finalVertex;
  }

  _ensurePositionWithinBounds(position, sceneBounds) {
    const transformedPosition = this._TransformVertex(position);

    // Ensure the position is within the scene bounds
    transformedPosition.x = Math.max(
      sceneBounds.minX,
      Math.min(transformedPosition.x, sceneBounds.maxX),
    );
    transformedPosition.y = Math.max(
      sceneBounds.minY,
      Math.min(transformedPosition.y, sceneBounds.maxY),
    );
  ...
  async Load({ url, fonts = null, progressCbk = null, workerFactory = null }) {
    ...
    const { scene, dxf, blockData } = await this.worker.Load(
      url,
      fonts,
      this.options,
      progressCbk,
    ...
    // console.log(
    //   "lib inside dxf viewer parsedDxf dxfviewer",
    //   JSON.stringify(dxf)
    // );
    ...
    //adding block bounding box to list
    if (dxf.blocks) {
      let minX = Infinity;
      let minY = Infinity;
      let maxX = -Infinity;
      ...
      // Traverse through each block in dxf.blocks
      for (const [key, block] of Object.entries(dxf.blocks)) {
        const blc = new Block(block);
        const blockCtx = blc.DefinitionContext();
        // Initialize an array to hold vertices for the current block
        const verticesList = [];

        // Traverse through each entity in the block
        if (block.entities) {
          for (const entity of block.entities) {
            // Traverse through each vertex array in the entity
            if (entity.vertices) {
              for (const vertex of entity.vertices) {
                // Assuming vertex is objects with x and y coordinates

                const newVertex = this._TransformVertex(vertex, blockCtx);
                verticesList.push({
                  x: newVertex.x,
                  y: newVertex.y,
                });
        ...
        if (verticesList.length > 0) {
          ...
          // Traverse through vertices to find min and max values
          ...
          blockBoundingBoxMap.set(key, boundingBox);
        }
      }
    }
    ...
    for (const batch of scene.batches) {
      //console.log("onstart current dxf batch is ", batch);
      this._LoadBatch(scene, batch);
    }

    const sceneBounds = {
      minX: scene.bounds.minX - scene.origin.x,
      maxX: scene.bounds.maxX - scene.origin.x,
      minY: scene.bounds.minY - scene.origin.y,
      maxY: scene.bounds.maxY - scene.origin.y,
    };
    ///render all block bounding box inside the scene
    this._setupAllBoundingBox(sceneBounds);

    this._setupHelper(sceneBounds);
    ...
    hover.subscribe("hover", (hover) => {
      console.log("current hover value is ", hover);
      if (hover) {
        this.helperBox.position.copy(hover.position);
        this.helperBox.visible = true;
      } else {
    ...
    //   this.boundingBoxMeshList
    // );
    // select.subscribe("select", (selects) =>
    //   console.log("Selected entities", selects)
    // );
  ...
  _setupAllBoundingBox(sceneBounds) {
    allBlockEntity.forEach((entity) => {
      console.log(
        "Current object entity is = ",
        JSON.stringify(entity),
        " Counter ",
        this.counter,
      );
      const boundingBox = blockBoundingBoxMap.get(entity.name);
      ...
      if (boundingBox) {
        ...
        // Position the plane in the scene at the entity position
        const position = { x: entity.position.x, y: entity.position.y };

        const entityPosition = this._ensurePositionWithinBounds(
          position,
          sceneBounds,
        );

        planeMesh.position.set(entityPosition.x, entityPosition.y, 0);
        planeMesh.scale.set(entity?.xScale ?? 1, entity?.yScale ?? 1, 1);
        const xScale = entity?.xScale ?? 1;
        const yScale = entity?.yScale ?? 1;
        planeMesh.scale.set(xScale, yScale, 1);
        const rotationDegrees = entity.rotation ?? 90;
        const rotationRadians = THREE.MathUtils.degToRad(rotationDegrees);

        //Apply rotation to the planeMesh
        //planeMesh.rotation.z = rotationRadians;

        planeMesh.userData = entity;
        planeMesh.visible = false;
        this.boundingBoxMeshList.push(planeMesh);
        this.scene.add(planeMesh);
      }
    });
  }

  getObjectSizeInWorldUnits(sceneBound, sizeInPixels, renderer) {
    const width = renderer.domElement.clientWidth;
    const height = renderer.domElement.clientHeight;

    const frustumWidth = sceneBound.maxX - sceneBound.minX;
    const frustumHeight = sceneBound.maxY - sceneBound.minY;

    const objectWidth = (sizeInPixels * frustumWidth) / width;
    const objectHeight = (sizeInPixels * frustumHeight) / height;
  ...
  GetBoundingBoxes() {
    return this.boundingBoxMeshList;
  }
...
function createBaseGeometry(thickness) {
  return new THREE.CylinderGeometry(thickness, thickness, 1, 8);
}

function addBlockToScene() {}

// function createHitboxes(verticesArray, thickness) {
//   const baseGeometry = createBaseGeometry(thickness);
//   const material = new THREE.MeshBasicMaterial({
//     color: 0xff0000,
//     visible: true,
//   });

//   if (verticesArray.length % 4 === 0) {
//     console.log("inside hit box exact vertices divisible by 4 ");
//   } else {
//     console.log("inside hit box not exact vertices not divisible by 4 ");
//   }

//   const count = verticesArray.length / 4;
//   const instancedMesh = new THREE.InstancedMesh(baseGeometry, material, count);

//   const dummy = new THREE.Object3D();

//   for (let i = 0; i < verticesArray.length; i += 4) {
//     const start = new THREE.Vector3(verticesArray[i], verticesArray[i + 1], 0);
...
//     dummy.updateMatrix();
//     instancedMesh.setMatrixAt(i / 4, dummy.matrix);
//   }
...
class Batch {
  ...
  constructor(viewer, scene, batch) {
    ...
    if (
      ...
      //console.log("current key layer data ", this.key);
      if (layer) {
  ...
  *_CreateObjects(instanceBatch) {
    const color = instanceBatch
      ? instanceBatch._GetInstanceColor(this.key.color)
      : this.key.color;

    //console.log("currentcreateobject block name ", this.key.blockName);
    //XXX line type
    ...
    switch (this.key.geometryType) {
      case BatchingKey.GeometryType.POLYLINE:
    ...
    function CreateObject(vertices, indices, userData, geometryType) {
      ...
      const vertexArray = vertices.array;

      // if (geometryType === 3 || geometryType === 4) {
      // } else {
      //   const hitbox = createHitboxes(vertexArray, 1);
    ...
    const userData = {
      ...
      blockName: this.key.blockName ?? this.key.name,
    };
    ...
    if (this.chunks) {
      ...
      yield CreateObject(this.vertices, null, userData, this.key.geometryType);
    }
  }
