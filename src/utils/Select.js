...
export class Select extends Raycaster {
  constructor(container, camera, boundingBoxes, raycasting = null) {
    super();
    ...
    //init raycasting
    this._initRaycasting(container, camera, boundingBoxes, raycasting);
    ...
    this.container.addEventListener(
      "pointerdown",
      async (e) => await this._onPointerDown(e),
      false,
    );
    this.container.addEventListener(
      "pointerup",
      async (e) => await this._onPointerUp(e),
      false,
    );
    this.container.addEventListener(
      "pointermove",
      async (e) => await this._onPointerMove(e),
      false,
    );
  }
  ...
  async _onPointerMove(event) {
    if (this._onSelectionBox) {
      ...
      this.drawSelectionBox(
        this._onSelectionBox.start,
        this._onSelectionBox.end,
        rect,
      );
      return;
  ...
  drawSelectionBox(start, end, rect) {
    ...
    this._selectionBox.style.background = "blue";
  ...
  _getEntitiesUnderSelectionBox(start, end) {
    ...
    this._boxHelpers.start3dpoint = new Vector3(start.x, start.y, 0).unproject(
      this.camera,
    );
    this._boxHelpers.end3dpoint = new Vector3(end.x, end.y, 0).unproject(
      this.camera,
    );
  ...
  select(obj, material = null) {
    ...
    objs.forEach((o) => {
      ...
      clone.selected = true;
      clone.traverse((c) => {
        if (c.material) c.material = material ? material : this._material;
      });
      o.parent.add(clone);
