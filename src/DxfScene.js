...
export class DxfScene {
  ...
  async Build(dxf, fontFetchers) {
    ...
    console.log("current whole dxf data is ", JSON.stringify(dxf));
    ...
    if (dxf.blocks) {
      for (const [, block] of Object.entries(dxf.blocks)) {
        console.log("adding data to block is ", JSON.stringify(block));
    ...
    for (const entity of dxf.entities) {
      ...
      if (entity.type === "INSERT") {
        ...
        //console.log("current block data insert is", JSON.stringify(block));
    ...
    for (const entity of dxf.entities) {
      ...
      //const blockCtx = { name: entity.name };
    ...
    const dxfData = _mapToObject(this.blocks);
    delete this.blocks;
    return { newDxf: dxfData };
  }
  ...
  async _FetchFonts(dxf) {
    ...
    const ProcessEntity = async (entity) => {
      ...
      if (
        ...
        ret = await this.textRenderer.FetchFonts(
          ParseSpecialChars(entity.text),
        );
  ...
  _ProcessEntity(entity, blockCtx = null) {
    //console.log("current enity dataa is ", JSON.stringify(entity));
    switch (entity.type) {
  ...
  *_DecomposeLine(entity, blockCtx) {
    ...
    const layer = this._GetEntityLayer(entity, blockCtx) ?? entity.layer;
    const color = this._GetEntityColor(entity, blockCtx);
    yield new Entity({
      entityType: entity.type,
      handle: entity.handle,
      ownerHandle: entity.ownerHandle,
      name: blockCtx?.name ?? entity.name,
  ...
  *_DecomposeText(entity, blockCtx) {
    ...
    for (const renderEntity of renderEntities) {
      renderEntity.entityType = entity.type; // Ensure entityType is set
      yield renderEntity;
    }
  }
    ...
    const dim = new LinearDimension(
      {
        ...
        angle: entity.angle,
        text: entity.text,
        textAnchor: entity.middleOfText
          ? new Vector2().copy(entity.middleOfText)
          : null,
  ...
  *_DecomposeHatch(entity, blockCtx) {
    if (entity.isSolid) {
      console.log(
        "current solid hatch entity is hatch = ",
      ...
      for (
        let loopIndex = 0;
        loopIndex < entity.boundaryLoops.length;
        loopIndex++
      ) {
        const loop = entity.boundaryLoops[loopIndex];

        if (loop.type !== 1) {
          console.warn(`Unsupported boundary loop type: ${loop.type}`);
          continue;
        }
        ...
        for (let edgeIndex = 0; edgeIndex < loop.edges.length; edgeIndex++) {
          ...
          if (edge.type === 1) {
            // Line segment
            vertices.push({ x: edge.start?.x, y: edge.start?.y });
            if (edgeIndex === loop.edges.length - 1) {
              vertices.push({ x: edge.end?.x, y: edge.end?.y });
            }
          } else if (edge.type === 2) {
            // Arc segment
            vertices.push({ x: edge.start?.x, y: edge.start?.y });
            // You may want to add intermediate points for the arc here
            if (edgeIndex === loop.edges.length - 1) {
              vertices.push({ x: edge.end?.x, y: edge.end?.y });
            }
          } else {
            console.warn(`Unsupported edge type: ${edge.type}`);
          }
        }

        if (vertices.length >= 2) {
          console.log(
            "adding entity to yield in hatch detection = ",
            entity.type,
          );
          yield new Entity({
            entityType: entity.type,
      ...
      if (
        style != HatchStyle.ODD_PARITY &&
        style != HatchStyle.THROUGH_ENTIRE_AREA
      ) {
        console.log(
          "_DecomposeHatch not HatchStyle.ODD_PARITY  ",
          entity,
          " blockCtx ",
          blockCtx,
        );
      ...
      console.log(
        "current boundary loops are = ",
        JSON.stringify(boundaryLoops),
      );
      if (boundaryLoops.length == 0) {
        console.warn(
          "HATCH entity with empty boundary loops array " +
      ...
      if (entity.patternName) {
        ...
        if (!pattern) {
          console.log(
            `Hatch pattern with name ${entity.patternName} not found ` +
      ...
      console.log(
        "inside decompose hatch transform value is not null entity = ",
        JSON.stringify(pattern),
      );
      const seedPoints = entity.seedPoints
        ? entity.seedPoints
        : [{ x: 0, y: 0 }];
        ...
        for (const line of pattern.lines) {
          ...
          for (let lineIdx = minLineIdx; lineIdx <= maxLineIdx; lineIdx++) {
            ...
            function RenderSegment(seg) {
              ...
              console.log("adding entity for decomposing hatch seg = ", seg);
            ...
            if (dashPatLength !== null) {
              ...
              if (maxSegIdx - minSegIdx >= MAX_HATCH_SEGMENTS) {
                console.warn(
                  "Too many segments produced by hatching pattern line",
                );
                continue;
              }
      ...
      console.log(
        "_DecomposeHatch at end seedPoint = ",
        seedPoints,
        " blockCtx ",
        blockCtx,
      );
      ...
      if (loop.type & 2) {
        ...
        for (const edge of loop.edges) {
          switch (edge.type) {
            ...
            default:
              console.warn(
                "Unhandled hatch boundary loop edge type: " + edge.type,
              );
          }
        }
      }
  ...
  _ProcessInsert(entity, blockCtx = null) {
    ...
    this._UpdateBounds(
      new Vector2(bounds.minX, bounds.minY).applyMatrix3(transform),
    );
    this._UpdateBounds(
      new Vector2(bounds.maxX, bounds.maxY).applyMatrix3(transform),
    );
    this._UpdateBounds(
      new Vector2(bounds.minX, bounds.maxY).applyMatrix3(transform),
    );
    this._UpdateBounds(
      new Vector2(bounds.maxX, bounds.minY).applyMatrix3(transform),
    );
    ...
    if (block.flatten) {
      for (const batch of block.batches) {
        this._FlattenBatch(entity, batch, layer, color, lineType, transform);
      }
    } else {
      const key = new BatchingKey(
  ...
  *_DecomposePolyline(entity, blockCtx = null) {
    console.log("current entity is polyline ", JSON.stringify(entity));
    ...
    // console.log(
    //   "dxf scene vertices decompose polyline initial vert ",
    //   JSON.stringify(entity.vertices)
    // );
    if (entity.includesCurveFitVertices || entity.includesSplineFitVertices) {
      entityVertices = entity.vertices.filter(
        (v) => v.splineVertex || v.curveFittingVertex,
      );
    ...
    function* CommitSegment(endIdx) {
      ...
      if (curPlainLine) {
        ...
        yield* _this._GenerateShapedPolyline(
          entity.type,
          entity.handle,
          entity.ownerHandle,
          blockCtx?.name ?? entity.name,
          vertices,
          layer,
          color,
          curLineType,
  ...
  *_DecomposeSpline(entity, blockCtx = null) {
  ...
  _ProcessPolyline(entity, blockCtx = null) {
    ...
    if (verticesCount <= 3) {
      ...
      if (entity.shape && verticesCount > 2) {
        batch.PushVertex(
          this._TransformVertex(entity.vertices[verticesCount - 1], blockCtx),
        );
  ...
  _ProcessTriangles(entity, blockCtx = null) {
    ...
    if (entity.indices.length % 3 !== 0) {
      console.error(
        "Unexpected size of indices array: " + entity.indices.length,
      );
      return;
    }
    ...
    //*** splitting into chunks is not yet implemented. Currently used only for text glyphs so
  ...
  _GetEntityExtrusionTransform(entity) {
    if (entity.type === "HATCH") {
      if (entity.hasOwnProperty("extrusionDirection")) {
        console.log(
          "current entity data for hatch detection extrusionDirection exist = ",
          JSON.stringify(entity),
        );
      }
    }
  ...
  _GetBatch(key) {
    ...
    if (batch !== null) {
      batch.key.entityType = key.entityType;
  ...
  _TransformVertex(v, blockCtx = null) {
    ...
    // console.log(
    //   "inside _transform vertex initial vertex ",
    //   v,
    //   " final vertex ",
    //   finalVertex
    // );
    ...
    if (this.key.IsIndexed()) {
      ...
      for (const chunk of batch.chunks) {
        ...
        for (let i = 0; i < verticesSize; i += 2) {
          const v = new Vector2(
            chunk.vertices.Get(i),
            chunk.vertices.Get(i + 1),
          );
          if (transform) {
            v.applyMatrix3(transform);
          }
...
class IndexedChunkWriter {
  ...
  Finish() {
    if (this.numVerticesPushed !== this.verticesCount) {
      throw new Error(
        `Not all vertices pushed: ${this.numVerticesPushed}/${this.verticesCount}`,
      );
    }
  }
}
