...
export const OrbitControls = function (object, domElement) {
  ...
  function getZoomScale(isMouseMovement) {
    return Math.pow(
      0.95,
      scope.zoomSpeed * (isMouseMovement ? scope.mouseZoomSpeedFactor : 1),
    );
  }
  ...
  function zoomPan(zoomRatio, zoomCenter) {
    const panX =
      ((zoomCenter.x - scope.domElement.width / 2) * (zoomRatio - 1)) /
      zoomRatio;
    const panY =
      ((zoomCenter.y - scope.domElement.height / 2) * (zoomRatio - 1)) /
      zoomRatio;
    pan(-panX, -panY);
  }
